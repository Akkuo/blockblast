...
## 六、 物理特效與演出防呆 (VFX & Animation Polishing)
* **死局雙階段實體演出 (Deadlock Wave & Explosion)**：
  - **Phase 1 (盤面填補)**：保留死亡盤面，針對空格由下往上以極短延遲（波浪狀）填補一般方塊（磚塊 1~7 隨機交錯），直至滿格。
  - **Phase 2 (物理炸飛)**：全盤無延遲同時向上拔地而起。方塊具備 Ease-In 向上高加速度（重力反轉），並附帶隨機水平兩側散開與旋轉。
* **防殘影與幀數規範 (Anti-Smearing)**：高速旋轉的方塊，角速度嚴格限制在 `vr < 0.15`，避免 60fps 下的貼圖殘影。
* **結算 UI 生命週期交接 (UI Timing & Lifecycle)**：Game Over Modal 必須精準等待（約 45 幀）物理動畫完全飛出螢幕後才彈出，嚴禁提早切斷演出。
* **開發與測試工具 (DevTools)**：UI 頂部需設置獨立的「TEST WAVE」按鈕，允許一鍵觸發死局特效以供 QA 驗證。
